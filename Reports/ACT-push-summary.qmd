...
push_dates <- tar_read(act_push_dates)
```
...
natural_earth <- read_geoparquet_sf(tar_read('natural_earth'), quiet = T)
...
station_summary |> 
  ...
  tab_footnote('statons listed from west to east',
                locations = cells_title()) |> 
  as_raw_html()
```

```{r}
midatl <- read_geoparquet_sf(tar_read('midatl'))
bathy <- tar_read('md_bathy') |> 
  arrow::open_dataset() |> 
  dplyr::filter(CONTOUR %in% seq(-50, -5, by = 5)) |> 
  geoarrow_collect_sf() |> 
  st_transform(4326)



station_spatial <- tar_read(station_key) |> 
...
ggplot() +
  ...
  labs(subtitle = 'Records by Mid-Bay receivers') +
